...
double
ActualClass::getValue()
{
    return this->value_;
}

double
ActualClass::add(double toAdd)
{
    this->value_ += toAdd;
    return this->value_;
}
...
private:
    static Napi::FunctionReference constructor; // reference to store the class definition
                                                // that needs to be exported to JS
    ...
    ActualClass *actualClass_; // internal instance of actualclass used to perform actual
                               // operations.
};
...
Napi::Object
ClassExample::Init(Napi::Env env, Napi::Object exports)
{
    Napi::HandleScope scope(env);

    Napi::Function func =
        DefineClass(env, "ClassExample",
                    {
...
Napi::Value
ClassExample::GetValue(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
...
Napi::Value
ClassExample::Add(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
